...
describe('postgresql introspection stopgaps', () => {
  ...
  describe('postgres partioned 35 - deferred constraint found', () => {
...
describeIf(!process.env.TEST_SKIP_COCKROACHDB)('cockroachdb stopgaps', () => {
  ...
  const computeSetupParams = (warningCode: number, variant?: number): SetupParams => {
    const setupParams: SetupParams = {
      ...
      dirname: path.join(
        ...
        'introspection',
        'cockroachdb',
        `row-ttl-warning-${warningCode}${variant ? `-${variant}` : ''}`,
      ),
    }
    return setupParams
  }

  const setupCockroachForWarning = (warningCode: number, variant?: number) => {
    const setupParams = computeSetupParams(warningCode, variant)
  ...
  describe('cockroachdb warning 31 - row ttl found', () => {
    const warningCode = 31
    setupCockroachForWarning(warningCode)

    test('basic introspection', async () => {
      ctx.fixture(`introspection/cockroachdb/row-ttl-warning-${warningCode}`)
      const introspect = new DbPull()
      const result = introspect.parse(['--print'])
